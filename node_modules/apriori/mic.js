    ...
    (function(Apriori) {
        ...
        var Algorithm = (function() {
            ...
            Algorithm.prototype.analyze = function(transactions) {
                ...
                ////convert 
                var convertResult = ArrayUtils.convert(oneElementItemSets, transactions);
                ...
                while (currentLItemSets.length !== 0) {
                    ...
                    var findResultItemSets = self.findItemSetsMinSupportSatisfied(joinedSets, clearTransactions, self.minSupport, frequencies, transactionsLenght, convertResult);
            ...
            //
            Algorithm.prototype.findItemSetsMinSupportSatisfied = function(itemSets, transactions, minSupport, frequencies, transactionsLenght, convertResult) {
                ...
                if(!convertResult){
                    ...
                    for(var i = 0; i < itemSets.length; i++){
                        var subSets = Combinatorics.combination(itemSets[i], itemSets[i].length -1).toArray();
                        var minPosition = ArrayUtils.getMinPosition(subSets, convertResult);
                        for(var j = 0; j < minPosition.length; j++){
                            if (ArrayUtils.isSubSetArrayOf(itemSets[i], transactions[minPosition[j]])) {
                                if (!frequencies[itemSet.toString()])
                                    frequencies[itemSet.toString()] = 0;
                                if (!localFrequencies[itemSet.toString()])
                                    localFrequencies[itemSet.toString()] = 0;
        ...
        var ArrayUtils = (function() {
            ...
            ArrayUtils.toFixedSizeJoinedSets = function(itemSets, length) {
                ...
                for (var e = 0; e < joinedSetArray.length; e++) {
                    ...
                    console.log(sunKItemSets, "sunKItemSets");
            ...
            ArrayUtils.convert = function(itemSetsTarget , transactions){
                ...
                for(var i = 0; i < itemSetsTarget.length; i++){
                    ...
                    for(var j = 0; j < transactions.length; j++){
                        // var index = transactions[j].indexOf(itemSetsTarget[i].itemSet[0]);
            ...
            ArrayUtils.getMinPosition = function(itemSets, convertResult){
                for(var i = 1; i < itemSets.length; i++){
                    var itemSet = itemSets[i];
                    var minLen = convertResult[0].position.length;
                    var row = convertResult[0].position;
                    for(var j = 0; j < convertResult.length; j++){
                        // if(convertResult[0])
                        if(convertResult[j].position.length - minLen < 0){
                            minLen = convertResult[itemSet].position;
                        }
                    }
                }
                return minLen;
